...
def clean_intra_noise(root_folder, feature_path, fea_dims, threshold):
    ...
    for key in label_featurelist_dict:
        ...
        for i in range(fea_num):
            ...
            for j in range(fea_num):
                ...
                sim = np.dot(x_vec_1, x_vec_2) / (np.linalg.norm(x_vec_1, ord=2) * np.linalg.norm(x_vec_2, ord=2) + + 0.0000001)
                if sim > 0.6:
    ...
    for key in label_featurelist_dict:
        ...
        for i in range(len(cur_fea_list)):
            ...
            sim = np.dot(x_vec, center) / (np.linalg.norm(x_vec, ord=2) * np.linalg.norm(center, ord=2) + 0.0000001)
